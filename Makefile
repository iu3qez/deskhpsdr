...
##############################################################################
#
# Header dependencies generated by $(DEPFLAGS) during the last build.
# They do not exist on a clean tree, hence the leading "-".
#
##############################################################################

ALL_OBJS=$(OBJS) $(AUDIO_OBJS) $(USBOZY_OBJS) $(TCI_OBJS) \
	$(MIDI_OBJS) $(STEMLAB_OBJS) $(SATURN_OBJS) $(TTS_OBJS)

DEPFILES=$(ALL_OBJS:.o=.d) src/version.d
