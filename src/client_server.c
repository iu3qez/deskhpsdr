...
/* STRLCPY macro - use GLib's g_strlcpy */
#define STRLCPY(dst, src, size) g_strlcpy(dst, src, size)

/* Stub functions for CLIENT-only functionality (not needed for server-only) */
static int ext_set_title(void *data) { (void)data; return 0; }
static int ext_rx_remote_update_display(void *data) { (void)data; return 0; }
static int ext_remote_set_zoom(void *data) { (void)data; return 0; }
static int ext_remote_set_pan(void *data) { (void)data; return 0; }

/* Stub for radio_remote_start - CLIENT-only functionality */
int radio_remote_start(void *data) {
  (void)data;
  t_print("radio_remote_start: CLIENT mode not supported in this build\n");
  return 0;
}
