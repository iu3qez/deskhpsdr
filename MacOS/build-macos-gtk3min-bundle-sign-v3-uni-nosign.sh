...
if ! security find-identity -v -p codesigning | grep -Fq "$IDENTITY"; then
  ...
  if [ "$PUBLISH" = "1" ]; then
    ...
    echo "       PUBLISH=1 requires Developer ID signing and notarization."
    exit 1
  fi
  echo "WARNING: signing identity not found: $IDENTITY"
  echo "         Building unsigned app bundle; signing and notarization will be skipped."
fi
...
if [ "$SIGNING_AVAILABLE" = "1" ]; then
  ...
  echo "Skipping Developer ID signing and Gatekeeper assessment."
fi
