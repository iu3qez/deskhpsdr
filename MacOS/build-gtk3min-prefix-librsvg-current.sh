...
patch_glib_pipe2_for_macos26() {
  local meson_file="${SRCROOT}/glib-${GLIB_VER}/meson.build"

  if [ "$(uname -s)" = "Darwin" ] && [ "${MACOSX_DEPLOYMENT_TARGET}" = "26.0" ]; then
    if grep -q "^[[:space:]]*'pipe2',[[:space:]]*$" "${meson_file}"; then
      sed -i.bak "/^[[:space:]]*'pipe2',[[:space:]]*$/d" "${meson_file}"
    fi
  fi
}

check_glib_pipe2_for_macos26() {
  local config_h="${SRCROOT}/glib-${GLIB_VER}/build/config.h"

  if [ "$(uname -s)" = "Darwin" ] && [ "${MACOSX_DEPLOYMENT_TARGET}" = "26.0" ]; then
    if grep -q '^#define HAVE_PIPE2 1$' "${config_h}"; then
      echo "ERROR: GLib detected pipe2 although deployment target is macOS 26.0"
      exit 1
    fi
  fi
}
...
fetch_unpack "https://download.gnome.org/sources/glib/2.88/glib-${GLIB_VER}.tar.xz" "glib-${GLIB_VER}.tar.xz" "glib-${GLIB_VER}"
...
fetch_unpack "https://download.gnome.org/sources/gdk-pixbuf/2.44/gdk-pixbuf-${GDK_PIXBUF_VER}.tar.xz" "gdk-pixbuf-${GDK_PIXBUF_VER}.tar.xz" "gdk-pixbuf-${GDK_PIXBUF_VER}"
...
meson_build_install "gdk-pixbuf-${GDK_PIXBUF_VER}" --prefix="${PREFIX}" --libdir=lib --buildtype=release -Dpng=enabled -Djpeg=enabled -Dtiff=enabled -Dgif=enabled -Dglycin=disabled -Dandroid=disabled -Dothers=disabled -Dintrospection=disabled -Ddocumentation=false -Dman=false -Dtests=false -Dinstalled_tests=false -Dthumbnailer=disabled
update_gdk_pixbuf_cache
